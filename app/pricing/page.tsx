...
function PricingCard({ plan, cat }: { plan: Plan; cat: Category }) {
  return (
    <div
    ...
    >
      ...
      <div className="flex-1">
        ...
        <p className="mt-1.5 text-sm text-white/55 leading-relaxed">{plan.description}</p>

        <div className="mt-5">
          <span className={`text-2xl font-bold ${plan.featured ? cat.accent : "text-white"}`}>
            {plan.price}
          </span>
          {plan.priceNote && (
            <p className="mt-0.5 text-xs text-white/35">{plan.priceNote}</p>
          )}
        </div>
      ...
      <div className="mt-6 flex flex-col gap-2">
        {plan.cta === "demo" && !plan.customPrice ? (
          <>
            ...
            <Link
              href="/contact"
              className="block text-center rounded-full py-2 text-xs font-medium text-white/50 hover:text-white/80 transition"
            >
          ...
          <Link
            href="/contact"
            className="block text-center rounded-full py-2.5 text-sm font-semibold border border-white/20 text-white hover:bg-white/[0.08] transition-all hover:scale-[1.02]"
          >
            Request Quote
          </Link>
        )}
...
function FaqItem({ faq, open, onToggle }: { faq: { q: string; a: string }; open: boolean; onToggle: () => void }) {
  return (
    <div className="border-b border-white/10 last:border-0">
      ...
      >
        <span className={`text-sm font-semibold transition-colors ${open ? "text-white" : "text-white/80 hover:text-white"}`}>
          {faq.q}
        </span>
        <span className={`flex-shrink-0 flex h-5 w-5 items-center justify-center rounded-full border text-xs transition-all ${open ? "border-blue-500/50 bg-blue-500/15 text-blue-400 rotate-45" : "border-white/20 text-white/50"}`}>
          +
        </span>
      </button>
      {open && (
        <p className="pb-5 text-sm text-white/55 leading-relaxed">
          {faq.a}
        </p>
      )}
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#06080f] text-white">
      ...
      <main>
        ...
        {/* ── Hero ─────────────────────────────────────────────────────── */}
        <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(ellipse_at_top,rgba(59,130,246,0.15),transparent_55%)]">
          ...
          <div className="relative mx-auto max-w-5xl px-6 py-24 md:py-32 text-center">
            ...
            <div className="mt-10 flex flex-col sm:flex-row justify-center gap-3">
              ...
              <Link
                href="/contact"
                className="px-7 py-3.5 rounded-full border border-white/20 text-white text-sm font-semibold hover:bg-white/[0.07] transition-all hover:scale-105"
              >
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-3">
              {["256-bit SSL", "Stripe & Wise ready", "Verified U.S. LLC", "No payment until confirmed", "GDPR compliant"].map((t) => (
                <span key={t} className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/50">
        ...
        {/* ── Sticky category nav ───────────────────────────────────────── */}
        <div ref={navRef} className="sticky top-[72px] z-30 border-b border-white/10 bg-[#06080f]/95 backdrop-blur-xl">
        ...
        <div className="mx-auto max-w-5xl px-6 pt-10">
          <div className="flex gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/[0.06] px-5 py-4">
            ...
            <p className="text-xs text-amber-300/70 leading-relaxed">
              <span className="font-semibold text-amber-300">Pricing disclosure: </span>
        ...
        <section className="mx-auto max-w-7xl px-6 py-16">
          <div className="text-center mb-10">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-400 mb-3">How it works</p>
            <h2 className="text-2xl sm:text-3xl font-semibold">From browse to launch in 3 steps</h2>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            {[
              ...
              <div key={step} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/30 mb-1">Step {step}</p>
            ...
            <div key={cat.id} id={cat.id} className="scroll-mt-32">
              ...
              <div className={`mb-8 rounded-2xl border px-6 py-5 flex items-center gap-4 ${cat.accentBorder} ${cat.accentBg}`}>
                ...
                <div>
                  ...
                  <p className="text-sm text-white/55 mt-0.5">{cat.subtitle}</p>
                </div>
              </div>
              ...
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {cat.plans.map((plan) => (
                  <PricingCard key={plan.name} plan={plan} cat={cat} />
                ))}
              </div>
            </div>
          ))}
        ...
        <section className="mx-auto max-w-3xl px-6 py-16">
          <div className="text-center mb-10">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-400 mb-3">Common questions</p>
            <h2 className="text-2xl sm:text-3xl font-semibold">Pricing FAQ</h2>
          </div>

          <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-6 divide-y divide-white/10">
            {FAQS.map((faq, i) => (
              <FaqItem
                key={i}
                faq={faq}
        ...
        <section className="mx-auto max-w-5xl px-6 pb-10">
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] px-6 py-5 text-xs text-white/40 leading-relaxed space-y-2">
            <p className="font-semibold text-white/60">Important Pricing Notes</p>
            ...
            <p>
              Questions?{" "}
              <a href="mailto:sales@shatasolutions.com" className="text-white/60 hover:text-blue-400 transition-colors">sales@shatasolutions.com</a>
              {" · "}
              <a href="mailto:billing@shatasolutions.com" className="text-white/60 hover:text-blue-400 transition-colors">billing@shatasolutions.com</a>
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-7xl px-6 pb-20">
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-blue-600/10 via-white/[0.03] to-purple-600/10 p-10 md:p-16 text-center">
            <div className="pointer-events-none absolute inset-0">
            ...
            <div className="relative">
              ...
              <div className="mt-8 flex flex-col sm:flex-row justify-center gap-3">
                <Link
                  href="/contact"
                  className="px-7 py-3.5 bg-white text-black rounded-full text-sm font-semibold hover:bg-white/90 transition-all hover:scale-105"
                >
                  Request a Free Quote
                </Link>
                <Link
                  href="/checkout-demo"
                  className="px-7 py-3.5 border border-blue-500/40 bg-blue-500/10 text-blue-400 rounded-full text-sm font-semibold hover:bg-blue-500/20 transition-all hover:scale-105"
                >
                  Try Checkout Demo
                </Link>
                <Link
                  href="/services"
                  className="px-7 py-3.5 border border-white/20 text-white rounded-full text-sm font-semibold hover:bg-white/[0.07] transition-all hover:scale-105"
                >
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-5 text-xs text-white/40">
                <a href="mailto:sales@shatasolutions.com" className="hover:text-white/70 transition">sales@shatasolutions.com</a>
                <a href="tel:+16197761222" className="hover:text-white/70 transition">+1 (619) 776-1222</a>
