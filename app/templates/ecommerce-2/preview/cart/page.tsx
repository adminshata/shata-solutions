...
import { Breadcrumbs, Container, EmptyState, Quantity, SectionHeading } from "@/components/templates/shata-home/ui/Atoms";
  ...
  return (
    <>
      <Container className="pt-8">
        <Breadcrumbs items={[{ label: "Home", href: "/templates/ecommerce-2/preview" }, { label: "Cart" }]} />
      </Container>
      ...
      <Container className="pb-20">
        {cart.cart.lines.length === 0 ? (
          ...
          <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr]">
            <ul className="divide-y divide-[color:var(--store-border)] border border-[color:var(--store-border)] bg-[color:var(--store-surface)]">
              {cart.cart.lines.map((line) => {
                ...
                return (
                  <li key={sig} className="flex gap-4 p-4">
                    <div className="relative h-24 w-24 flex-none overflow-hidden border border-[color:var(--store-border)] bg-[color:var(--store-bg)]">
                    ...
                    <div className="flex flex-1 flex-col">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <Link
                            ...
                            className="text-sm font-bold uppercase tracking-wide text-[color:var(--store-fg)] hover:underline"
                          >
                            {product.name}
                          </Link>
                        ...
                        <span className="text-sm font-bold text-[color:var(--store-accent)]">{formatPrice(product.price * line.quantity)}</span>
                      </div>
                      <div className="mt-auto flex items-center justify-between pt-3">
                        ...
                        <button
                          ...
                          className="text-[10px] font-bold uppercase tracking-[0.18em] text-[color:var(--store-muted)] hover:text-[color:var(--store-fg)]"
                        >
                          Remove
                        </button>
                      </div>
            ...
            <aside className="border border-[color:var(--store-border)] bg-[color:var(--store-surface)] p-6 lg:sticky lg:top-24 lg:h-fit">
              ...
              <div className="mt-4 flex items-center justify-between border-t border-[color:var(--store-border)] pt-4 text-base">
                ...
                <span className="font-bold text-[color:var(--store-accent)]">{formatPrice(cart.subtotal)}</span>
              </div>
              <div className="mt-2 text-[11px] text-[color:var(--store-muted)]">{cart.shippingHint}</div>
              <LinkButton href="/templates/ecommerce-2/preview/checkout" full size="lg" className="mt-5">
                Proceed to checkout →
              </LinkButton>
              <LinkButton href="/templates/ecommerce-2/preview/shop" variant="outline" full size="md" className="mt-2">
                Continue shopping
              </LinkButton>
            </aside>
          </div>
