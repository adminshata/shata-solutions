...
import { Badge } from "@/components/templates/supermarket1/ui/Atoms";
import { useSite, useCart } from "@/lib/supermarket1/context";
import { findProduct, formatPrice, activeProducts } from "@/lib/supermarket1/utils";
    ...
    return (
      <SiteShell>
        ...
        <main className="py-20 text-center" style={{ background: "#F3F4F6" }}>
          ...
          <Link href={`${BASE_PATH}/shop`} className="text-sm font-semibold underline" style={{ color: "#629D23" }}>
            Back to shop
          </Link>
        </main>
        <Footer />
      ...
      <main style={{ background: "#F3F4F6" }}>
        <div className="container mx-auto px-4 py-8">
          ...
          <nav className="flex items-center gap-1 text-xs text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href={`${BASE_PATH}/shop`} className="hover:text-[#629D23]">Shop</Link>
            <span>/</span>
            ...
            <div className="grid gap-8 md:grid-cols-2">
                    ...
                    {product.images.map((img, i) => (
                      <button
                        ...
                        className={`relative h-14 w-14 rounded border-2 overflow-hidden transition-colors ${
                          i === activeImg ? "border-[#629D23]" : "border-gray-200"
                        }`}
                      >
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-4">
                  <span className="text-3xl font-extrabold" style={{ color: "#629D23" }}>
                ...
                <div className="mt-6 flex items-center gap-3">
                  <div className="flex items-center rounded border border-gray-200">
                    <button
                      ...
                      className="px-3 py-2 text-base font-bold text-[#629D23] hover:bg-gray-50"
                    >
                      −
                    </button>
                ...
                <div className="mt-6 space-y-2 border-t border-gray-100 pt-4 text-xs text-gray-500">
                  ...
                  {product.category && (
                    <div>
                      ...
                      <Link href={`${BASE_PATH}/categories/${product.category}`} className="hover:underline" style={{ color: "#629D23" }}>
                        {product.category.replace(/-/g, " ")}
                      </Link>
                    </div>
                  )}
