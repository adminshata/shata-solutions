...
import { SUPERMARKET3_DEFAULTS } from "@/lib/supermarket3/defaults";
import type { SiteConfig } from "@/lib/supermarket3/types";
...
import { listAllSupermarket3Images, listProductImages, checkSupermarket3Images } from "./actions";

const STORAGE_KEY = "supermarket3/config-draft";
const PRIMARY = "#1D6CE3";
    ...
    <div className="flex h-screen overflow-hidden bg-[#0f1117] text-white" style={themeVars(config.theme) as CSSProperties}>
      ...
      <div className="flex w-[420px] shrink-0 flex-col border-r border-white/10">
        ...
        <div className="border-t border-white/10 p-3">
          <button
            type="button"
            onClick={() => { setConfig(SUPERMARKET3_DEFAULTS); window.localStorage.removeItem(STORAGE_KEY); }}
      ...
      <div className="flex-1 overflow-y-auto bg-white">
        <Supermarket3Provider config={config}>
          <SiteShell>
            <Header />
            <main>
              <BannerSlider />
...
const inputCls =
  "w-full rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white outline-none focus:border-[#1D6CE3] transition-colors";
