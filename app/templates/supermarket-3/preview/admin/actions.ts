...
const BASE_PUBLIC = ["public", "templates", "supermarket3"];
...
export async function listAllSupermarket3Images(): Promise<string[]> {
  const [products, category, banner, feature, blog] = await Promise.all([
    listDir(DIRS.products, "/templates/supermarket3/products"),
    listDir(DIRS.category, "/templates/supermarket3/category"),
    listDir(DIRS.banner,   "/templates/supermarket3/banner"),
    listDir(DIRS.feature,  "/templates/supermarket3/feature"),
    listDir(DIRS.blog,     "/templates/supermarket3/blog"),
  ]);
  return [...products, ...category, ...banner, ...feature, ...blog];
}

export async function listProductImages(): Promise<string[]> {
  return listDir(DIRS.products, "/templates/supermarket3/products");
}

export async function checkSupermarket3Images(imagePaths: string[]): Promise<{
  checked: number;
  missing: string[];
}> {
  const unique = [...new Set(imagePaths.filter((src) => src.startsWith("/templates/supermarket3/")))];
