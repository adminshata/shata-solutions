...
import { findCategory, productsByCategory } from "@/lib/supermarket3/utils";
        ...
        <section className="py-8">
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              <Link
                href={`${BASE_PATH}/shop`}
                className="rounded-full border border-gray-200 bg-white px-4 py-1.5 text-xs font-semibold text-gray-600 hover:border-[#1D6CE3] hover:text-[#1D6CE3] transition-colors"
              >
                All
              </Link>
              {config.categories.filter(c => c.active !== false).map((cat) => (
                <Link
                  ...
                  className={`rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${
                    cat.handle === handle
                      ? "border-[#1D6CE3] text-white"
                      : "border-gray-200 bg-white text-gray-600 hover:border-[#1D6CE3] hover:text-[#1D6CE3]"
                  }`}
                  style={cat.handle === handle ? { background: "#1D6CE3" } : {}}
                >
                  {cat.name}
                </Link>
              ))}
