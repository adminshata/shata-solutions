...
function OrderContent() {
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("sm3/order");
  ...
  return (
    <div className="max-w-lg mx-auto bg-white rounded border border-gray-200 p-8 text-center">
      ...
      {order && (
        <div className="mt-6 text-left border border-gray-100 rounded p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between font-bold text-gray-800 border-t border-gray-100 pt-2 mt-2">
            ...
            <span style={{ color: "#1D6CE3" }}>{formatPrice(order.total)}</span>
          </div>
        </div>
      )}
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
        ...
        <Link
          href={`${BASE_PATH}/shop`}
          className="rounded border border-gray-200 px-5 py-2.5 text-sm font-bold text-gray-700 hover:border-[#1D6CE3] hover:text-[#1D6CE3] transition-colors"
        >
          Continue Shopping
        </Link>
      </div>
