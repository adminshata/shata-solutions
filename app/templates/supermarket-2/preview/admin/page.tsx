...
import { SUPERMARKET2_DEFAULTS, SUPERMARKET2_STORAGE_KEY } from "@/lib/supermarket2/defaults";
...
import type {
  ...
  StoreCategory,
  StoreVendor,
} from "@/lib/supermarket2/types";
import { themeVars } from "@/lib/supermarket2/utils";
import { checkSupermarket2Images, listAllSupermarket2Images, listProductImages } from "./actions";
...
function normalizeSiteConfig(input: unknown): SiteConfig {
  if (!input || typeof input !== "object") return SUPERMARKET2_DEFAULTS;

  const draft = input as Partial<SiteConfig>;
  const defaults = SUPERMARKET2_DEFAULTS;
...
function normalizeProduct(product: unknown, index: number): SiteConfig["products"][number] {
  const fallback = SUPERMARKET2_DEFAULTS.products[index % SUPERMARKET2_DEFAULTS.products.length];
  ...
  const images = Array.isArray(row.images)
    ...
    : image
      ? [image.startsWith("/") ? image : `/templates/supermarket2/products/${image}`]
      : fallback.images;
...
export default function Supermarket2AdminPage() {
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(SUPERMARKET2_STORAGE_KEY);
      if (raw) setConfig(normalizeSiteConfig(JSON.parse(raw)));
    } catch {
      window.localStorage.removeItem(SUPERMARKET2_STORAGE_KEY);
    }
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        window.localStorage.setItem(SUPERMARKET2_STORAGE_KEY, JSON.stringify(config));
  ...
  function reset() {
    setConfig(SUPERMARKET2_DEFAULTS);
    window.localStorage.removeItem(SUPERMARKET2_STORAGE_KEY);
  ...
  return (
    <div
      className="sm2-admin min-h-screen bg-[#10141c] text-white"
    ...
    >
      <style>{`
        ...
        .sm2-admin .admin-sidebar-button {
          padding: 18px 20px !important;
        }
        .sm2-admin .admin-sidebar-button-title {
          display: block;
          font-size: 18px !important;
          line-height: 1.25 !important;
          font-weight: 900 !important;
        }
        .sm2-admin .admin-sidebar-button-helper {
          display: block;
          margin-top: 7px;
        ...
        .sm2-admin .admin-panel {
          padding: 28px !important;
        }
        .sm2-admin .admin-field-label {
          display: block;
          margin-bottom: 10px;
        ...
        .sm2-admin .admin-input {
          min-height: 58px;
          font-size: 18px !important;
          line-height: 1.5 !important;
          color: #111827 !important;
        }
        .sm2-admin textarea.admin-input {
          min-height: 150px;
        }
        .sm2-admin .admin-toggle-label {
          font-size: 18px !important;
          line-height: 1.35 !important;
        }
        .sm2-admin .admin-small-button {
          font-size: 14px !important;
          line-height: 1.2 !important;
        }
        .sm2-admin [class*="text-[10px]"],
        .sm2-admin [class*="text-xs"] {
          font-size: 14px !important;
          line-height: 1.45 !important;
        }
        .sm2-admin [class*="text-sm"] {
          font-size: 16px !important;
          line-height: 1.5 !important;
        }
      `}</style>
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#080c13]/95 backdrop-blur">
        <div className="flex flex-wrap items-center gap-4 px-6 py-4">
          ...
          <div className="min-w-0">
            <div className="text-sm font-bold uppercase tracking-[0.25em] text-[#fca5a5]">Template editor</div>
            <h1 className="text-2xl font-black leading-tight">{config.name} customization</h1>
          </div>
          <div className="ml-auto flex items-center gap-3">
            <span className="rounded-full border border-[#DC2626]/50 px-4 py-2 text-sm font-semibold text-[#fecaca]">
              {savedMsg}
            </span>
            <button
              type="button"
      ...
      <main className="grid min-h-[calc(100vh-82px)] lg:grid-cols-[330px_1fr]">
        <aside className="border-r border-white/10 bg-[#0b1018] p-5">
          <div className="mb-5 rounded-xl border border-white/10 bg-white/[0.04] p-5">
            <p className="text-base font-bold text-[#fca5a5]">Simple editor</p>
            ...
            <p className="mt-3 text-base leading-7 text-white/70">
              ...
              <Link href={`${BASE_PATH}/dashboard`} target="_blank" className="font-bold text-[#fca5a5]">
                /preview/dashboard
              </Link>.
            </p>
          </div>
        ...
        <section className="overflow-y-auto bg-[#f5f7fb] p-6 text-[#18212f]">
          <div className="mx-auto max-w-6xl">
            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-base font-bold uppercase tracking-[0.18em] text-[#DC2626]">{active.label}</p>
...
function BlogTab({ config, patch, imagePool }: EditorProps & { imagePool: string[] }) {
  return (
    <EditableList
      ...
      items={config.blog}
      addLabel="Add post"
      create={() => ({ id: `post-${Date.now()}`, title: "New post", slug: "new-post", image: imagePool[0] ?? "", excerpt: "", content: "", author: "QuickMart Team", date: "May 6, 2026", category: "Grocery", active: true })}
  ...
  return (
    <Panel title={title}>
      <div className="space-y-5">
        ...
        <button type="button" onClick={() => onChange([...items, create()])} className="rounded-xl bg-[#DC2626] px-5 py-3 text-base font-black text-white">
          {addLabel}
        </button>
      </div>
    </Panel>
...
function ToggleField({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
      <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-5 w-5 accent-[#DC2626]" />
...
const inputCls = "admin-input w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-lg text-slate-950 outline-none focus:border-[#DC2626] focus:ring-4 focus:ring-[#DC2626]/10";
const smallBtnCls = "admin-small-button rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:border-[#DC2626]";
