...
export function generateStaticParams() {
  return agency1Portfolio.map((p) => ({ slug: p.slug }));
}
...
export default function Agency1PortfolioDetailsPage({
  ...
  const proj = agency1Portfolio.find((p) => p.slug === params.slug);
  if (!proj) notFound();

  const related = agency1Portfolio.filter((p) => p.slug !== params.slug).slice(0, 3);

  return (
    <>
      <Agency1Header />
      <main>
        <Agency1Breadcrumb title={proj.title} subtitle={proj.category} />
