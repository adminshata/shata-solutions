...
export default function Agency1ServiceDetailsPage() {
  ...
  const service = config.services.find((s) => s.slug === handle);
  const others = config.services.filter((s) => s.slug !== handle).slice(0, 3);

  return (
    <>
      <Agency1Header />
      <main>
        <Agency1Breadcrumb title={service?.title ?? "Not found"} subtitle="Service Detail" />
              ...
              <div className="col-xl-8">
                {!service ? (
                  <div style={{ background: "#fff", borderRadius: 16, padding: 32 }}>
                    <h2 style={{ fontWeight: 800, marginBottom: 12 }}>Service not found</h2>
                    <p style={{ color: "#666", lineHeight: 1.7 }}>
                      This service doesn&apos;t exist. Return to the services list.
                    </p>
                    <div style={{ marginTop: 20 }}>
                      <Link href="/templates/agency-1/preview/services" style={{ color: "#F14F44", fontWeight: 700 }}>
                        ← Back to Services
                      </Link>
                    </div>
                  </div>
