...
import { Badge } from "@/components/templates/supermarket5/ui/Atoms";
import { useSite, useCart } from "@/lib/supermarket5/context";
import { findProduct, formatPrice, activeProducts } from "@/lib/supermarket5/utils";
      ...
      <main style={{ background: "#F3F4F6" }}>
        <div className="container mx-auto px-4 py-8">
          ...
          <nav className="flex items-center gap-1 text-xs text-gray-500 mb-6">
            <Link href={BASE_PATH} className="hover:text-[#7C3AED]">Home</Link>
            <span>/</span>
            <Link href={`${BASE_PATH}/shop`} className="hover:text-[#7C3AED]">Shop</Link>
            <span>/</span>
                    ...
                    {product.images.map((img, i) => (
                      <button
                        ...
                        className={`relative h-14 w-14 rounded border-2 overflow-hidden transition-colors ${
                          i === activeImg ? "border-[#7C3AED]" : "border-gray-200"
                        }`}
                      >
                ...
                <div className="mt-6 flex items-center gap-3">
                  <div className="flex items-center rounded border border-gray-200">
                    <button
                      ...
                      className="px-3 py-2 text-base font-bold text-[#7C3AED] hover:bg-gray-50"
                    >
                      −
                    </button>
                ...
                <div className="mt-6 space-y-2 border-t border-gray-100 pt-4 text-xs text-gray-500">
                  ...
                  {product.category && (
                    <div>
                      ...
                      <Link href={`${BASE_PATH}/categories/${product.category}`} className="hover:underline" style={{ color: "#7C3AED" }}>
                        {product.category.replace(/-/g, " ")}
                      </Link>
                    </div>
                  )}
