...
export default function Page({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const vendorName = handle === "fresh-juice-bar" ? "Fresh Juice Bar" : handle.split("-").map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join(" ");
  const vendorDescription = handle === "fresh-juice-bar"
          ...
          <div className="row g-5">
            <div className="col-lg-4">
              <div style={{ background: '#fff', borderRadius: 8, padding: 32, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', textAlign: 'center' }}>
                <img src="/templates/supermarket5/vendor/01.jpg" alt="vendor" style={{ width: 100, height: 100, borderRadius: '50%', objectFit: 'cover', marginBottom: 16 }} onError={e => { (e.target as HTMLImageElement).src = '/templates/supermarket5/products/01.jpg'; }} />
                <h4>{vendorName}</h4>
