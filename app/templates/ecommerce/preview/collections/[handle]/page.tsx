...
import { findCategory, productsInCategory } from "@/lib/shata-store/utils";
import { Breadcrumbs, Container, EmptyState, SectionHeading } from "@/components/templates/shata-store/ui/Atoms";
      ...
      <section className="border-b border-[color:var(--store-border)]">
        <Container className="grid items-center gap-10 py-10 md:grid-cols-2">
          <div>
            ...
            <div className="mt-3 text-sm text-[color:var(--store-muted)]">
              {items.length} {items.length === 1 ? "piece" : "pieces"} available
            </div>
          </div>
          <div className="relative aspect-[16/9] overflow-hidden rounded-[var(--store-radius)] border border-[color:var(--store-border)] bg-[color:var(--store-surface)]">
      ...
      <Container className="py-10">
        {items.length === 0 ? (
          <EmptyState
            ...
            copy="This collection doesn't have any products yet. Check back soon, or browse the full shop."
