...
export default function ProductDetailPage({ params }: { params: { handle: string } }) {
  ...
  const handleAdd = () => addToCart({ id: Date.now(), image: `/templates/supermarket4/products/${product.image}`, title: product.title ?? "Product", price: parseFloat(product.price ?? "0"), quantity, active: true });
  const handleWishlist = () => addToWishlist({ id: Date.now(), image: `/templates/supermarket4/products/${product.image}`, title: product.title ?? "Product", price: parseFloat(product.price ?? "0"), quantity: 1 });
            ...
            <div className="col-lg-6">
              <div className="product-thumb-area">
                <div className="thumb-wrapper one filterd-items figure">
                  <div className="product-thumb"><img src={`/templates/supermarket4/products/${product.image}`} alt={product.title} style={{ width: "100%", borderRadius: 8 }} /></div>
                </div>
              </div>
            </div>
            <div className="col-lg-6">
