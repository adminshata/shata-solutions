...
const BASE_PUBLIC = ["public", "templates", "supermarket4"];
...
export async function listAllSupermarket4Images(): Promise<string[]> {
  const [products, category, banner, feature, blog] = await Promise.all([
    listDir(DIRS.products, "/templates/supermarket4/products"),
    listDir(DIRS.category, "/templates/supermarket4/category"),
    listDir(DIRS.banner,   "/templates/supermarket4/banner"),
    listDir(DIRS.feature,  "/templates/supermarket4/feature"),
    listDir(DIRS.blog,     "/templates/supermarket4/blog"),
  ]);
  return [...products, ...category, ...banner, ...feature, ...blog];
}

export async function listProductImages(): Promise<string[]> {
  return listDir(DIRS.products, "/templates/supermarket4/products");
}

export async function checkSupermarket4Images(imagePaths: string[]): Promise<{
  checked: number;
  missing: string[];
}> {
  const unique = [...new Set(imagePaths.filter((src) => src.startsWith("/templates/supermarket4/")))];
