...
import { SUPERMARKET4_DEFAULTS, SUPERMARKET4_STORAGE_KEY } from "@/lib/supermarket4/defaults";
import { buildProductsSchema } from "@/lib/supermarket4/products-manager-schema";
import type {
  HeaderNavConfig,
  HeroSlide,
  HomeSection,
  ...
  StoreCategory,
  StoreVendor,
} from "@/lib/supermarket4/types";
import { themeVars } from "@/lib/supermarket4/utils";
import { checkSupermarket4Images, listAllSupermarket4Images, listProductImages } from "./actions";
...
function normalizeSiteConfig(input: unknown): SiteConfig {
  if (!input || typeof input !== "object") return SUPERMARKET4_DEFAULTS;

  const draft = input as Partial<SiteConfig>;
  const defaults = SUPERMARKET4_DEFAULTS;
...
function normalizeProduct(product: unknown, index: number): SiteConfig["products"][number] {
  const fallback = SUPERMARKET4_DEFAULTS.products[index % SUPERMARKET4_DEFAULTS.products.length];
  ...
  const images = Array.isArray(row.images)
    ...
    : image
      ? [image.startsWith("/") ? image : `/templates/supermarket4/products/${image}`]
      : fallback.images;
...
export default function Supermarket4AdminPage() {
  const [config, setConfig] = useState<SiteConfig>(SUPERMARKET4_DEFAULTS);
  ...
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem(SUPERMARKET4_STORAGE_KEY);
      if (raw) setConfig(normalizeSiteConfig(JSON.parse(raw)));
    } catch {
      window.localStorage.removeItem(SUPERMARKET4_STORAGE_KEY);
    }
    listAllSupermarket4Images().then(setImagePool).catch(() => {});
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        window.localStorage.setItem(SUPERMARKET4_STORAGE_KEY, JSON.stringify(config));
  ...
  useEffect(() => {
    ...
    const id = setTimeout(() => {
      checkSupermarket4Images(paths).then(setImageReport).catch(() => {
  ...
  function reset() {
    setConfig(SUPERMARKET4_DEFAULTS);
    window.localStorage.removeItem(SUPERMARKET4_STORAGE_KEY);
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#080c13]/95 backdrop-blur">
        <div className="flex flex-wrap items-center gap-4 px-6 py-4">
          ...
          <div className="ml-auto flex items-center gap-3">
            <span className="rounded-full border border-[#F97316]/50 px-4 py-2 text-sm font-semibold text-[#fed7aa]">
              {savedMsg}
            </span>
            <button
              type="button"
        ...
        <section className="overflow-y-auto bg-[#f5f7fb] p-6 text-[#18212f]">
          <div className="mx-auto max-w-6xl">
            <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-base font-bold uppercase tracking-[0.18em] text-[#F97316]">{active.label}</p>
...
function BlogTab({ config, patch, imagePool }: EditorProps & { imagePool: string[] }) {
  return (
    <EditableList
      ...
      items={config.blog}
      addLabel="Add post"
      create={() => ({ id: `post-${Date.now()}`, title: "New post", slug: "new-post", image: imagePool[0] ?? "", excerpt: "", content: "", author: "OrangeMart Team", date: "May 6, 2026", category: "Grocery", active: true })}
  ...
  return (
    <Panel title={title}>
      <div className="space-y-5">
        ...
        <button type="button" onClick={() => onChange([...items, create()])} className="rounded-xl bg-[#F97316] px-5 py-3 text-base font-black text-white">
          {addLabel}
        </button>
      </div>
    </Panel>
...
function ToggleField({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <label className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-4">
      <input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} className="h-5 w-5 accent-[#F97316]" />
...
const inputCls = "admin-input w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-lg text-slate-950 outline-none focus:border-[#F97316] focus:ring-4 focus:ring-[#F97316]/10";
const smallBtnCls = "admin-small-button rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:border-[#F97316]";
