  ...
  return (
    <ReferencePageShell title={post.title ?? "Blog detail"} subtitle={`${post.category} • ${post.publishedDate}`}>
      <article style={{ background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 8px 30px rgba(0,0,0,.06)" }}>
        <img src={`/templates/supermarket4/blog/${post.bannerImg ?? post.image}`} alt={post.title} style={{ width: "100%", maxHeight: 460, objectFit: "cover" }} />
        <div style={{ padding: 32, lineHeight: 1.9, fontSize: 18 }}>
          <p>{post.descripTion}</p>
          <p>OrangeMart shares practical food ideas, seasonal shopping notes, vendor stories, and grocery tips for everyday home cooking.</p>
        </div>
      </article>
    </ReferencePageShell>
  );
