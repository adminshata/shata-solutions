...
import { SHATA_HOME_DEFAULTS } from "./defaults";
...
export function StoreProvider({
  ...
  const value = config ?? SHATA_HOME_DEFAULTS;
  return (
  ...
  switch (action.type) {
    ...
    case "add": {
      ...
      if (existingIdx >= 0) {
        ...
        next.push({ productId: action.productId, options: action.options, quantity: action.quantity });
      }
      return { lines: next };
    }
    ...
    case "update": {
      if (action.quantity <= 0) {
        return { lines: cart.lines.filter((l) => lineSignature(l.productId, l.options) !== action.signature) };
      }
      return {
        lines: cart.lines.map((l) =>
          lineSignature(l.productId, l.options) === action.signature ? { ...l, quantity: action.quantity } : l
        ),
      };
    }
...
const CART_STORAGE_KEY_PREFIX = "shata-home/cart";
...
function CartProvider({ children, config }: { children: ReactNode; config: StoreConfig }) {
  ...
  const value: CartContextValue = useMemo(
    () => ({
      ...
      resolveProduct,
      add: (productId, options, quantity = 1) => dispatch({ type: "add", productId, options, quantity }),
