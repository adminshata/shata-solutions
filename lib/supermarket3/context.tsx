...
import { SUPERMARKET3_DEFAULTS } from "./defaults";
...
export function useSite(): SiteContextValue {
  ...
  if (!ctx) throw new Error("useSite must be used within Supermarket3Provider");
  return ctx;
}
...
function CartProvider({ children, config }: { children: ReactNode; config: SiteConfig }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("sm3_cart");
      if (stored) setCartItems(JSON.parse(stored) as CartItem[]);
    } catch {
      localStorage.removeItem("sm3_cart");
    }
    try {
      const storedLines = localStorage.getItem("sm3_cart_lines");
      if (storedLines) setLines(JSON.parse(storedLines) as CartLine[]);
    } catch {
      localStorage.removeItem("sm3_cart_lines");
    }
    setIsCartLoaded(true);
  }, []);

  useEffect(() => {
    if (isCartLoaded) localStorage.setItem("sm3_cart", JSON.stringify(cartItems));
  }, [cartItems, isCartLoaded]);

  useEffect(() => {
    if (isCartLoaded) localStorage.setItem("sm3_cart_lines", JSON.stringify(lines));
  }, [lines, isCartLoaded]);
...
function WishlistProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("sm3_wishlist");
      if (stored) setWishlistItems(JSON.parse(stored) as WishlistItem[]);
    } catch {
      localStorage.removeItem("sm3_wishlist");
    }
    setIsWishlistLoaded(true);
  }, []);

  useEffect(() => {
    if (isWishlistLoaded) localStorage.setItem("sm3_wishlist", JSON.stringify(wishlistItems));
...
function CompareProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("sm3_compare");
      if (stored) setCompareItems(JSON.parse(stored) as CompareItem[]);
    } catch {
      localStorage.removeItem("sm3_compare");
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("sm3_compare", JSON.stringify(compareItems));
  }, [compareItems]);
...
export function Supermarket3Provider({
  children,
  config = SUPERMARKET3_DEFAULTS,
}: {
  children: ReactNode;
  config?: SiteConfig;
}) {
