...
export function useSite(): SiteConfig {
  ...
  if (!ctx) throw new Error("useSite must be used inside <Supermarket3Provider>");
  return ctx;
}
...
function SiteProvider({
  ...
  const value = config ?? SUPERMARKET3_DEFAULTS;
  return (
...
export const Supermarket3Provider = SiteProvider;
...
const CART_KEY = "supermarket3/cart";
...
export function useCart(): CartContextValue {
  ...
  if (!ctx) throw new Error("useCart must be used inside <Supermarket3Provider>");
  return ctx;
}
