...
export function useSite(): SiteContextValue {
  ...
  if (!ctx) throw new Error("useSite must be used within Supermarket2Provider");
  return ctx;
}
...
function CartProvider({ children, config }: { children: ReactNode; config: SiteConfig }) {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.removeItem("sm2_cart");
    }
    try {
      const storedLines = localStorage.getItem("sm2_cart_lines");
      if (storedLines) setLines(JSON.parse(storedLines) as CartLine[]);
    } catch {
      localStorage.removeItem("sm2_cart_lines");
    }
    setIsCartLoaded(true);
  }, []);

  useEffect(() => {
    if (isCartLoaded) localStorage.setItem("sm2_cart", JSON.stringify(cartItems));
  }, [cartItems, isCartLoaded]);

  useEffect(() => {
    if (isCartLoaded) localStorage.setItem("sm2_cart_lines", JSON.stringify(lines));
  }, [lines, isCartLoaded]);
...
function WishlistProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      localStorage.removeItem("sm2_wishlist");
    }
    setIsWishlistLoaded(true);
  }, []);

  useEffect(() => {
    if (isWishlistLoaded) localStorage.setItem("sm2_wishlist", JSON.stringify(wishlistItems));
...
function CompareProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("sm2_compare");
      if (stored) setCompareItems(JSON.parse(stored) as CompareItem[]);
    } catch {
      localStorage.removeItem("sm2_compare");
    }
  }, []);

  useEffect(() => {
    localStorage.setItem("sm2_compare", JSON.stringify(compareItems));
  }, [compareItems]);
