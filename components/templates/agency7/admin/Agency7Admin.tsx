...
import { AGENCY7_DEFAULTS } from "../../../../lib/agency7/defaults";
import { useAgency7 } from "../../../../lib/agency7/context";
import type {
  Agency7Config,
  Agency7HeroBanner,
  Agency7HomeSection,
...
const blankListItem = (prefix: string): Agency7ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency7ListItem;
  update: (value: Partial<Agency7ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency7Admin() {
  const { config, setConfig, resetConfig } = useAgency7();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency7Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency7NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-7/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency7HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency7HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency7Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency7/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata Cyber Security Agency Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY7_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={7} page="cyber-security.html" title="Shata Cyber Security Agency live preview" />
        </section>
      </div>
    </main>
  );
