...
import { AGENCY5_DEFAULTS } from "../../../../lib/agency5/defaults";
import { useAgency5 } from "../../../../lib/agency5/context";
import type {
  Agency5Config,
  Agency5HeroBanner,
  Agency5HomeSection,
...
const blankListItem = (prefix: string): Agency5ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency5ListItem;
  update: (value: Partial<Agency5ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency5Admin() {
  const { config, setConfig, resetConfig } = useAgency5();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency5Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency5NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-5/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency5HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency5HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency5Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency5/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata Marketing Agency Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY5_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={5} page="digital-marketing-agency.html" title="Shata Marketing Agency live preview" />
        </section>
      </div>
    </main>
  );
