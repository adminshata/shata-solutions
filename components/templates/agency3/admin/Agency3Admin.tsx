...
import { AGENCY3_DEFAULTS } from "../../../../lib/agency3/defaults";
import { useAgency3 } from "../../../../lib/agency3/context";
import type {
  Agency3Config,
  Agency3HeroBanner,
  Agency3HomeSection,
...
const blankListItem = (prefix: string): Agency3ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency3ListItem;
  update: (value: Partial<Agency3ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency3Admin() {
  const { config, setConfig, resetConfig } = useAgency3();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency3Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency3NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-3/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency3HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency3HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency3Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency3/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata IT Solutions Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY3_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={3} page="it-solution.html" title="Shata IT Solutions live preview" />
        </section>
      </div>
    </main>
  );
