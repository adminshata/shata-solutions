...
import { AGENCY4_DEFAULTS } from "../../../../lib/agency4/defaults";
import { useAgency4 } from "../../../../lib/agency4/context";
import type {
  Agency4Config,
  Agency4HeroBanner,
  Agency4HomeSection,
...
const blankListItem = (prefix: string): Agency4ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency4ListItem;
  update: (value: Partial<Agency4ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency4Admin() {
  const { config, setConfig, resetConfig } = useAgency4();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency4Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency4NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-4/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency4HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency4HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency4Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency4/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata Software Agency Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY4_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={4} page="sofware.html" title="Shata Software Agency live preview" />
        </section>
      </div>
    </main>
  );
