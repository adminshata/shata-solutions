    ...
    <footer style={{ background: "#f3f4f6" }} className="border-t border-gray-200">
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {/* Col 1: About */}
          <div>
            <h3 className="text-lg font-bold mb-4" style={{ color: "#DC2626" }}>{config.logo.text}</h3>
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2 text-sm text-gray-600">
                <svg className="h-4 w-4 mt-0.5 shrink-0 text-[#DC2626]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {["Delivery Info", "Privacy Policy", "Terms & Conditions", "Customer Support", "Careers"].map((item) => (
                <li key={item}>
                  <Link href="#" className="text-sm text-gray-600 hover:text-[#DC2626] transition-colors flex items-center gap-1">
              ...
              {categories.map((cat) => (
                <li key={cat.id}>
                  <Link
                    ...
                    className="text-sm text-gray-600 hover:text-[#DC2626] transition-colors flex items-center gap-1"
                  >
                    <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                      <path d="M9 18l6-6-6-6" />
                    </svg>
          ...
          <div>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex gap-2">
              <input
                ...
                className="flex-1 rounded border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#DC2626]"
              />
              <button
                type="submit"
            ...
            <div className="flex items-center gap-3 mt-5">
              {[
                ...
                <a
                  ...
                  aria-label={s.label}
                  className="flex h-8 w-8 items-center justify-center rounded-full border border-gray-300 text-gray-500 hover:border-[#DC2626] hover:text-[#DC2626] transition-colors"
                >
