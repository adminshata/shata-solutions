  ...
  return (
    <header className={`w-full z-50 ${sticky ? "sticky top-0 shadow-md" : ""}`} style={{ background: "#fff" }}>
      ...
      {config.announcement.active && (
        <div style={{ background: "#DC2626" }} className="text-white">
      ...
      <div className="border-b border-gray-100">
        <div className="container mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <Link href={BASE_PATH} className="flex-1 lg:flex-none flex justify-center lg:justify-start">
            <span className="text-2xl font-extrabold tracking-tight" style={{ color: "#DC2626" }}>
              {config.logo.text}
            </span>
          </Link>
          ...
          <div className="flex-1 hidden md:block">
            <div className="relative">
              <input
                ...
                className="w-full rounded border border-gray-200 bg-gray-50 py-2.5 pl-4 pr-12 text-sm outline-none focus:border-[#DC2626] focus:ring-1 focus:ring-[#DC2626]"
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-2 flex-none">
            ...
            {/* Wishlist */}
            <button type="button" className="relative p-2 text-gray-600 hover:text-[#DC2626] transition-colors">
            ...
            <button
              ...
              className="relative flex items-center gap-2 rounded border border-[#DC2626] px-3 py-2 text-sm font-semibold text-[#DC2626] hover:bg-[#DC2626] hover:text-white transition-colors"
            >
      ...
      {searchOpen && (
        <div className="border-b border-gray-100 p-3 md:hidden">
          <input
            ...
            type="text"
            placeholder="Search for products..."
            className="w-full rounded border border-gray-200 bg-gray-50 py-2.5 px-4 text-sm outline-none focus:border-[#DC2626]"
          />
        </div>
      )}
              ...
              >
                {config.categories.filter(c => c.active !== false).map((cat) => (
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#DC2626]/10 hover:text-[#DC2626]"
                  >
                    {cat.name}
                  </Link>
                ))}
          ...
          <nav className="hidden md:flex items-center flex-1 px-2">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-3 text-sm font-medium text-gray-700 hover:text-[#DC2626] transition-colors"
              >
                {link.label}
              </Link>
            ))}
        ...
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b">
              <span className="text-lg font-bold" style={{ color: "#DC2626" }}>{config.logo.text}</span>
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-1">
              {[
                ...
                <Link
                  ...
                  className="block px-3 py-2.5 rounded text-sm font-medium text-gray-700 hover:bg-[#DC2626]/10 hover:text-[#DC2626]"
                >
                  {link.label}
                </Link>
              ))}
              <div className="pt-2 pb-1">
                ...
                {config.categories.filter(c => c.active !== false).map((cat) => (
                  <Link
                    ...
                    href={`${BASE_PATH}/categories/${cat.handle}`}
                    onClick={() => setMobileOpen(false)}
                    className="block px-3 py-2 text-sm text-gray-600 hover:text-[#DC2626]"
                  >
                    {cat.name}
                  </Link>
                ))}
