...
import { activeProducts, activeCategories } from "@/lib/supermarket2/utils";
import { ProductCard } from "../product/ProductCard";
import { SectionTitle } from "../ui/Atoms";

const BASE_PATH = "/templates/supermarket-2/preview";
const BASE_IMG = "/templates/supermarket2";
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {FEATURES.map((f) => (
            <div key={f.title} className="flex flex-col items-center text-center gap-2 p-4">
              <div className="text-[#DC2626]">{f.icon}</div>
        ...
        <div className="grid grid-cols-5 divide-x divide-gray-100">
          {cats.map((cat) => (
            <Link
              ...
              className="flex flex-col items-center gap-2 py-4 px-2 hover:bg-[#DC2626]/5 transition-colors"
            >
              <div className="relative h-10 w-10 rounded-full overflow-hidden bg-gray-100">
                {cat.image ? (
                  <Image src={cat.image} alt={cat.name} fill className="object-cover" sizes="40px" unoptimized />
                ) : (
                  <div className="h-full w-full bg-[#DC2626]/20 flex items-center justify-center text-[#DC2626] text-xs font-bold">
                    {cat.name[0]}
                  </div>
                )}
              </div>
    ...
    <section className="py-10">
      <div className="container mx-auto px-4">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link href={`${BASE_PATH}/shop`} className="text-sm font-semibold hover:underline" style={{ color: "#DC2626" }}>
            View All →
          </Link>
        </div>
    ...
    <section className="py-10">
      <div className="container mx-auto px-4">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link href="#" className="text-sm font-semibold hover:underline" style={{ color: "#DC2626" }}>
            All Articles →
          </Link>
        </div>
            ...
            <article key={post.id} className="bg-white rounded border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              <div className="relative h-44 bg-gray-100">
                ...
                <div className="absolute top-3 left-3">
                  <span className="rounded px-2 py-1 text-[10px] font-bold text-white" style={{ background: "#DC2626" }}>
                    {post.category}
                  </span>
                </div>
              </div>
              <div className="p-4">
                ...
                <Link href="#" className="text-xs font-bold hover:underline" style={{ color: "#DC2626" }}>
                  Read More →
                </Link>
              </div>
            </article>
