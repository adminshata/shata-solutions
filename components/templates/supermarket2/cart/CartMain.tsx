...
const CartMain = () => {
  ...
  useEffect(() => {
    const total = cartItems.reduce((acc, item) => acc + (isNaN(item.price) ? 0 : item.price * (item.quantity || 1)), 0);
    setSubtotal(total);
  }, [cartItems]);

  const applyCoupon = (e: React.FormEvent) => {
    e.preventDefault();
    if (coupon === '12345') { setDiscount(0.25); setCouponMessage('Coupon applied -25% successfully'); }
        ...
        <div className="row g-5">
            ...
            <div className="cart-area-main-wrapper">
              <div className="cart-top-area-note">
                ...
                <div className="bottom-content-deals mt--10">
                  <div className="single-progress-area-incard"><div className="progress"><div className="progress-bar wow fadeInLeft" role="progressbar" style={{ width: '80%' }} /></div></div>
                </div>
              </div>
            </div>
            <div className="rts-cart-list-area">
              ...
              {cartItems.map(item => (
                <div className="single-cart-area-list main item-parent" key={item.id}>
                  <div className="product-main-cart">
                    <div className="close section-activation" onClick={() => removeFromCart(item.id)}><i className="fa-regular fa-x" /></div>
                  ...
                  <div className="quantity">
                    <div className="quantity-edit">
                      ...
                      <div className="button-wrapper-action">
                        <button className="button minus" onClick={() => item.quantity > 1 && updateItemQuantity(item.id, item.quantity - 1)}><i className="fa-regular fa-chevron-down" /></button>
                        <button className="button plus" onClick={() => updateItemQuantity(item.id, item.quantity + 1)}><i className="fa-regular fa-chevron-up" /></button>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bottom-cupon-code-cart-area">
                <form onSubmit={applyCoupon}>
                  <input type="text" placeholder="Coupon Code" value={coupon} onChange={e => { setCoupon(e.target.value); setCouponMessage(''); }} />
          ...
          <div className="col-xl-3 col-12 order-1 order-xl-2">
            <div className="cart-total-area-start-right">
              ...
              <div className="subtotal"><span>Subtotal</span><h6 className="price">${subtotal.toFixed(2)}</h6></div>
              ...
              <div className="bottom">
                <div className="wrapper"><span>Total</span><h6 className="price">${finalTotal.toFixed(2)}</h6></div>
                <div className="button-area">
                  <Link href={`${BP}/checkout`} className="rts-btn btn-primary">Proceed To Checkout</Link>
                </div>
              </div>
            </div>
          </div>
