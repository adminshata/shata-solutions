...
import { AGENCY6_DEFAULTS } from "../../../../lib/agency6/defaults";
import { useAgency6 } from "../../../../lib/agency6/context";
import type {
  Agency6Config,
  Agency6HeroBanner,
  Agency6HomeSection,
...
const blankListItem = (prefix: string): Agency6ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency6ListItem;
  update: (value: Partial<Agency6ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency6Admin() {
  const { config, setConfig, resetConfig } = useAgency6();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency6Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency6NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-6/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency6HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency6HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency6Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency6/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata SEO Agency Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY6_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={6} page="seo-agency.html" title="Shata SEO Agency live preview" />
        </section>
      </div>
    </main>
  );
