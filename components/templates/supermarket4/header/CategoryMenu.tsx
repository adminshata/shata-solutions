        ...
        {menuItems.map((item, index) => (
          <li key={index}>
            <Link href="#" className="menu-item" onClick={(e) => { e.preventDefault(); if (item.submenu) toggleMenu(index); }}>
              <img src={`/templates/supermarket4/icons/${item.icon}`} alt="icons" />
            ...
            {item.submenu && (
              <ul className={`submenu mm-collapse ${openIndex === index ? "mm-show" : ""}`}>
                {item.submenu.map((sub, si) => (
                  <li key={si}><Link className="mobile-menu-link" href="/templates/supermarket-4/preview/shop">{sub}</Link></li>
                ))}
              </ul>
            )}
          </li>
