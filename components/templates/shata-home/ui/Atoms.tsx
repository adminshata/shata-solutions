  ...
  return (
    <header className={`flex flex-col gap-3 md:flex-row md:items-end md:justify-between ${isCenter ? "md:items-center md:flex-col" : ""}`}>
      <div className={isCenter ? "text-center" : ""}>
        {eyebrow && (
          <span className="inline-flex items-center gap-1.5 border-l-2 border-[color:var(--store-accent)] pl-3 text-[10px] font-bold uppercase tracking-[0.22em] text-[color:var(--store-accent)]">
            {eyebrow}
          </span>
        )}
        <h2 className="mt-2 max-w-2xl text-2xl font-black uppercase leading-tight tracking-tight text-[color:var(--store-fg)] md:text-3xl">
          {title}
        </h2>
        {subtitle && (
...
export function Price({
  ...
  return (
    <span className={`inline-flex items-baseline gap-2 ${className}`}>
      <span className={`font-black text-[color:var(--store-accent)] ${sizes[size].current}`}>
      ...
      {product.compareAtPrice && product.compareAtPrice > product.price && (
        <>
          ...
          {off !== null && (
            <span className="rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[color:var(--store-accent)]">
              -{off}%
            </span>
          )}
        </>
      ...
      <span className="flex gap-0.5">
        {stars.map((s) => (
          <svg key={s} width="12" height="12" viewBox="0 0 24 24"
            fill={s <= Math.round(value) ? "#f59e0b" : "transparent"}
...
export function ProductBadge({ kind }: { kind: NonNullable<Product["badge"]> }) {
  const map: Record<typeof kind, string> = {
    ...
    sale: "bg-[color:var(--store-accent)] text-white",
    bestseller: "bg-amber-400 text-slate-950",
    limited: "bg-gray-900 text-white",
  };
  const label = { new: "New", sale: "Sale", bestseller: "Top Pick", limited: "Limited" }[kind];
  return (
    <span className={`inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${map[kind]}`}>
      {label}
    </span>
  );
}
      ...
      <ol className="flex flex-wrap items-center gap-1">
        {items.map((item, i) => (
          <li key={i} className="flex items-center gap-1">
            {item.href ? (
              <Link href={item.href} className="hover:text-[color:var(--store-fg)]">{item.label}</Link>
            ) : (
...
export function Quantity({
  ...
  return (
    <div className={`inline-flex items-center border border-[color:var(--store-border)] bg-[color:var(--store-surface)] ${className}`}>
      <button type="button" onClick={() => onChange(Math.max(min, value - 1))} aria-label="Decrease quantity"
        className="h-10 w-10 text-base text-[color:var(--store-fg)] hover:bg-black/[0.03]">−</button>
      <span className="min-w-[2ch] px-1 text-center text-sm font-bold tabular-nums text-[color:var(--store-fg)]">{value}</span>
...
export function EmptyState({ title, copy, action }: { title: string; copy?: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center justify-center border border-dashed border-[color:var(--store-border)] bg-[color:var(--store-surface)] px-6 py-14 text-center">
      <div className="text-base font-bold text-[color:var(--store-fg)]">{title}</div>
