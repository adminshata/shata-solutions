...
import ProductDetails from '@/components/templates/supermarket5/modal/ProductDetails';
import CompareModal from '@/components/templates/supermarket5/modal/CompareModal';
import { useCart } from '@/lib/supermarket5/context';
import { useWishlist } from '@/lib/supermarket5/context';
...
const DealOfDayMain: React.FC<Props> = ({ Slug, ProductImage, ProductTitle, Price }) => {
  ...
  const handleAdd = () => addToCart({ id: Date.now(), image: `/templates/supermarket5/products/${ProductImage}`, title: ProductTitle ?? 'Product', price: Price ? parseFloat(Price) : 0, quantity: 1, active: true });

  const { addToWishlist } = useWishlist();
  const handleWishlist = () => addToWishlist({ id: Date.now(), image: `/templates/supermarket5/products/${ProductImage}`, title: ProductTitle ?? 'Product', price: parseFloat(Price ?? '0'), quantity: 1 });

  const { addToCompare } = useCompare();
  const handleCompare = () => addToCompare({ image: `/templates/supermarket5/products/${ProductImage}`, name: ProductTitle ?? 'Product', price: Price ?? '0', description: 'Premium grocery product.', rating: 5, ratingCount: 25, weight: '500g', inStock: true });
    ...
    <>
      ...
      <div className="image-and-action-area-wrapper">
        <Link href={`${BP}/shop/${Slug}`} className="thumbnail-preview">
          <img src={`/templates/supermarket5/products/${ProductImage}`} alt="grocery" />
        </Link>
      ...
      <ProductDetails show={activeModal === 'two'} handleClose={handleClose} productImage={`/templates/supermarket5/products/${ProductImage}`} productTitle={ProductTitle ?? 'Product'} productPrice={Price ?? '0'} />
    </>
  );
};
