        ...
        <div className="container mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <div className="hidden lg:flex items-center gap-4 text-xs text-gray-500 flex-none">
            <span className="flex items-center gap-1">
              <svg className="h-4 w-4 text-[#7C3AED]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          ...
          <Link href={BASE_PATH} className="flex-1 lg:flex-none flex justify-center lg:justify-start">
            <span className="text-2xl font-extrabold tracking-tight" style={{ color: "#7C3AED" }}>
              {config.logo.text}
            </span>
          </Link>
          ...
          <div className="flex-1 hidden md:block">
            <div className="relative">
              <input
                ...
                className="w-full rounded border border-gray-200 bg-gray-50 py-2.5 pl-4 pr-12 text-sm outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]"
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-2 flex-none">
            ...
            {/* Wishlist */}
            <button type="button" className="relative p-2 text-gray-600 hover:text-[#7C3AED] transition-colors">
            ...
            <button
              ...
              className="relative flex items-center gap-2 rounded border border-[#7C3AED] px-3 py-2 text-sm font-semibold text-[#7C3AED] hover:bg-[#7C3AED] hover:text-white transition-colors"
            >
      ...
      {searchOpen && (
        <div className="border-b border-gray-100 p-3 md:hidden">
          <input
            ...
            type="text"
            placeholder="Search for products..."
            className="w-full rounded border border-gray-200 bg-gray-50 py-2.5 px-4 text-sm outline-none focus:border-[#7C3AED]"
          />
        </div>
      )}
              ...
              >
                {config.categories.filter(c => c.active !== false).map((cat) => (
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-[#7C3AED]/10 hover:text-[#7C3AED]"
                  >
                    {cat.name}
                  </Link>
                ))}
          ...
          <nav className="hidden md:flex items-center flex-1 px-2">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-3 text-sm font-medium text-gray-700 hover:text-[#7C3AED] transition-colors"
              >
                {link.label}
              </Link>
            ))}
        ...
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b">
              <span className="text-lg font-bold" style={{ color: "#7C3AED" }}>{config.logo.text}</span>
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-1">
              {[
                ...
                <Link
                  ...
                  className="block px-3 py-2.5 rounded text-sm font-medium text-gray-700 hover:bg-[#7C3AED]/10 hover:text-[#7C3AED]"
                >
                  {link.label}
                </Link>
              ))}
              <div className="pt-2 pb-1">
                ...
                {config.categories.filter(c => c.active !== false).map((cat) => (
                  <Link
                    ...
                    href={`${BASE_PATH}/categories/${cat.handle}`}
                    onClick={() => setMobileOpen(false)}
                    className="block px-3 py-2 text-sm text-gray-600 hover:text-[#7C3AED]"
                  >
                    {cat.name}
                  </Link>
                ))}
