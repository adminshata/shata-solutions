...
import { AGENCY8_DEFAULTS } from "../../../../lib/agency8/defaults";
import { useAgency8 } from "../../../../lib/agency8/context";
import type {
  Agency8Config,
  Agency8HeroBanner,
  Agency8HomeSection,
...
const blankListItem = (prefix: string): Agency8ListItem => ({
  id: uniqueId(prefix),
  title: "New Item",
  slug: "new-item",
...
function ListItemForm({
  ...
  item: Agency8ListItem;
  update: (value: Partial<Agency8ListItem>) => void;
}) {
  return (
    <>
...
export default function Agency8Admin() {
  const { config, setConfig, resetConfig } = useAgency8();
  const [active, setActive] = useState<TabId>("brand");

  const updateConfig = (value: Partial<Agency8Config>) => setConfig((current) => ({ ...current, ...value }));
  ...
  const renderTab = () => {
    switch (active) {
      ...
      case "header":
        return (
          <>
            ...
            <ItemsManager<Agency8NavItem>
              title="Navigation"
              items={config.header.nav}
              onChange={(nav) => updateConfig({ header: { ...config.header, nav } })}
              create={() => ({ id: uniqueId("nav"), label: "New Link", href: "/templates/agency-8/preview", enabled: true })}
      ...
      case "home":
        return (
          <ItemsManager<Agency8HomeSection>
            title="Home Sections"
      ...
      case "hero":
        return (
          <ItemsManager<Agency8HeroBanner>
            title="Hero / Banners"
      ...
      case "testimonials":
        return (
          <ItemsManager<Agency8Testimonial>
            title="Testimonials"
            items={config.testimonials}
            onChange={(testimonials) => updateConfig({ testimonials })}
            create={() => ({ id: uniqueId("testimonial"), name: "New Client", role: "Client Role", quote: "Add the testimonial quote.", image: "/templates/agency8/imgs/home-2/testimonial/client-1.png", enabled: true })}
            renderForm={(item, update) => (
              <>
  ...
  return (
    <main style={S.page}>
      <header style={S.topbar}>
        <div>
          <h1 style={{ fontSize: 20, margin: 0 }}>Shata SaaS & Startups Editor</h1>
          <p style={{ margin: "4px 0 0", color: "#cbd5e1", fontSize: 13 }}>Drafts save automatically in localStorage.</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button type="button" style={{ ...S.ghost, background: "#fff" }} onClick={() => setConfig(AGENCY8_DEFAULTS)}>
            Load Defaults
          </button>
      ...
      <div style={S.shell}>
        ...
        <section style={S.preview}>
          <StaticAgencyFrame agencyNumber={8} page="sass.html" title="Shata SaaS & Startups live preview" />
        </section>
      </div>
    </main>
  );
