...
const normalizePhone = (phone, defaultCountry = "EG")=>{
    if (!phone) return "";
...
const saveLead = async (lead, newMessages, sessionId)=>{
    try {
        const supabase = getSupabase();
        // 🔍 find existing lead by session
        const { data: existingLead, error } = await supabase.from("leads").select("*").eq("session_id", sessionId).maybeSingle();
        if (error) {
        ...
        if (existingLead) {
            ...
            }).eq("id", existingLead.id).eq("session_id", sessionId);
        } else {
            // 🆕 create new lead
...
export async function POST(req) {
    try {
        ...
        let fallbackLeadId = null;
        if (!lead.email && lead.phone) {
            const { data } = await supabase.from("leads").select("id").eq("session_id", sessionId).is("phone", null).order("created_at", {
                ascending: false
            }).limit(1).maybeSingle();
            fallbackLeadId = data?.id || null;
        }
        if (!lead.phone && lead.email) {
            const { data } = await supabase.from("leads").select("id").eq("session_id", sessionId).is("email", null).order("created_at", {
                ascending: false
            }).limit(1).maybeSingle();
            fallbackLeadId = data?.id || null;
        }
        ...
        if (lead.email || lead.phone) {
            try {
                ...
                if (lead.email && lead.phone) {
                    const { data: emailRow } = await supabase.from("leads").select("id").eq("session_id", sessionId).eq("email", lead.email).maybeSingle();
                    const { data: phoneRow } = await supabase.from("leads").select("id").eq("session_id", sessionId).eq("phone", lead.phone).maybeSingle();
                    if (emailRow?.id && phoneRow?.id && emailRow.id !== phoneRow.id) {
                        // update email row with phone
                        await supabase.from("leads").update({
                            phone: lead.phone
                        }).eq("id", emailRow.id).eq("session_id", sessionId);
                        // delete duplicate phone row
                        await supabase.from("leads").delete().eq("id", phoneRow.id).eq("session_id", sessionId);
                    }
                }
                // 🔥 use fallback if no direct match
                if (!existingLead?.id && fallbackLeadId) {
                    const { error: fallbackUpdateError } = await supabase.from("leads").update({
                        email: lead.email ?? undefined,
                        phone: lead.phone ?? undefined,
                        created_at: new Date().toISOString()
                    }).eq("id", fallbackLeadId).eq("session_id", sessionId);
...
Never say you are an AI model. Act like a real sales assistant working for Shata Solutions.`
        ...
        const wantsWhatsApp = lowerMessage.includes("price") || lowerMessage.includes("cost") || lowerMessage.includes("start") || lowerMessage.includes("ready") || lowerMessage.includes("help") || lowerMessage.includes("contact") || lowerMessage.includes("call");
        // build WhatsApp number (use lead phone if exists, otherwise default)
        const whatsappNumber = lead.phone ? normalizePhone(lead.phone, detectCountry(message)) : "201010255736";
        ...
        try {
            const lowerMessage = message.toLowerCase();
            const isHotLead = lowerMessage.includes("price") || lowerMessage.includes("how much") || lowerMessage.includes("start") || lowerMessage.includes("ready") || lowerMessage.includes("now") || lowerMessage.includes("quote") || lowerMessage.includes("cost") || lowerMessage.includes("book") || lowerMessage.includes("call me") || lowerMessage.includes("lets start") || lowerMessage.includes("let's start");
...
- Make the email feel action-oriented`
        ...
        return new Response(JSON.stringify({
            reply: finalReply,
            leadCaptured: lead.email || lead.phone,
            lead
        }), {
            headers: {
                "Content-Type": "application/json"
